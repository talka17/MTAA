...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def sendResponse(self,code):
        ...
        for line in self.data:
            ...
            if line == "":
                break
        data.append("")
        text = "\r\n".join(data).encode() #string.join(data,"\r\n")
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar: #registrar.has_key(fromm):
                del registrar[fromm]
                self.sendResponse("200 Preslo v poriadku")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar: #registrar.has_key(origin):
            self.sendResponse("400 Cele zle")
            return
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination): #registrar.has_key(destination)
                ...
                self.sendResponse("406 Tak toto nie")
        else:
            self.sendResponse("500 Chyba servera")
